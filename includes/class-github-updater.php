<?php
/**
 * GitHub Releases updater — production ZIP assets from magpern/biopentra-loop-card.
 *
 ...
 */
final class Biopentra_Loop_Card_Github_Updater {

	private const API_LATEST = 'https://api.github.com/repos/magpern/biopentra-loop-card/releases/latest';

	private const PLUGIN_SLUG = 'biopentra-loop-card';

	private const TRANSIENT_RELEASE = 'biopentra_loop_card_github_release';
	...
	public static function is_enabled(): bool {
		if ( defined( 'BIOPENTRA_LOOP_CARD_DISABLE_GITHUB_UPDATER' ) && BIOPENTRA_LOOP_CARD_DISABLE_GITHUB_UPDATER ) {
			return false;
		}

		/** @var bool|null $filtered */
		$filtered = apply_filters( 'biopentra_loop_card_github_updater_enabled', null );
		if ( null !== $filtered ) {
	...
	public static function should_offer_update( string $installed, string $remote ): bool {
		...
		return (bool) apply_filters( 'biopentra_loop_card_github_updater_should_offer_update', $offer, $installed, $remote );
	}

	private function __construct() {
		$this->plugin_basename   = plugin_basename( BIOPENTRA_LOOP_CARD_FILE );
		$this->installed_version = defined( 'BIOPENTRA_LOOP_CARD_VER' )
			? (string) BIOPENTRA_LOOP_CARD_VER
			: '0.0.0';
	}
		...
		$response = wp_remote_get(
			...
			array(
				...
				'headers' => array(
					...
					'User-Agent' => 'biopentra-loop-card-updater/' . $this->installed_version,
				),
			)
		);
