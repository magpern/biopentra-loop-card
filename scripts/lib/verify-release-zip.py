#!/usr/bin/env python3
"""Verify biopentra-loop-card production release ZIP."""
...
PLUGIN_SLUG = "biopentra-loop-card"
VERSION_CONSTANT = "BIOPENTRA_LOOP_CARD_VER"
...
def segment_violations(path: str) -> list[str]:
    ...
    if parts:
        ...
        if leaf.startswith(".env"):
            hits.append(leaf)
        if leaf == "setup-shop-page-cli.php":
            hits.append(leaf)
...
def verify(zip_path: str, expected_version: str | None) -> int:
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        if not any(
            n.startswith(f"{root_prefix}vendor/plugin-update-checker/plugin-update-checker.php")
            for n in names
        ):
            print(
                "ERROR: missing vendor/plugin-update-checker/plugin-update-checker.php",
                file=sys.stderr,
            )
            return 1
