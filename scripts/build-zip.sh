#!/usr/bin/env bash
#
# Build production release ZIP: builds/biopentra-loop-card-{version}.zip
#
set -euo pipefail

readonly PLUGIN_SLUG="biopentra-loop-card"
...
readonly STAGE_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/blc-zip.XXXXXX")"
...
VERSION_CONST="$(
	grep -E "define\s*\(\s*'BIOPENTRA_LOOP_CARD_VER'" "${MAIN_FILE}" \
		| head -n 1 \
		| sed -E "s/.*'([^']+)'.*/\1/"
)"
...
echo "==> Biopentra Loop Card: build production ZIP"
...
tar -C "${REPO_ROOT}" \
	...
	--exclude='README.md' \
	--exclude='.gitignore' \
	--exclude='includes/setup-shop-page-cli.php' \
	-cf - . \
